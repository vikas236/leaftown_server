...
// Updated Property Types to match your requirements
...
export const users = pgTable("users", {
  ...
  displayId: serial("display_id"), // Auto-incrementing ID (e.g., USER-101)
  ...
  identityProofUrl: text("identity_proof_url"), // URL to uploaded ID card
...
export const properties = pgTable("properties", {
  ...
  // Basic Specs (Apartments/Houses)
  bedrooms: integer("bedrooms"),
  bathrooms: decimal("bathrooms", { precision: 5, scale: 1 }),
  furnishingStatus: furnishingEnum("furnishing_status").default("unfurnished"),
  ...
  // --- NEW: PROJECT / VENTURE FIELDS ---
  projectName: varchar("project_name", { length: 255 }), // e.g. "Green Valley Meadows"
  reraId: varchar("rera_id", { length: 100 }), // Regulatory ID
  approvalAuthority: varchar("approval_authority", { length: 100 }), // e.g. "HMDA", "DTCP", "GHMC"
  totalUnits: integer("total_units"), // Total plots/flats in the project
  projectType: varchar("project_type", { length: 50 }), // e.g. "Gated Community", "Open Plots"
...
export const supportTickets = pgTable("support_tickets", {
  id: uuid("id").defaultRandom().primaryKey(),
  displayId: serial("display_id"), // e.g. TKT-1002
...
// ... (Keep existing tables: savedListings, inquiries, showings, transactions) ...
...
// --- RELATIONS ---
// (Keep existing relations + Add Support Tickets)
...
export const usersRelations = relations(users, ({ one, many }) => ({
  ...
  tickets: many(supportTickets), // User has many tickets
}));
...
// (Add Inquiries/Showings/SavedListings/Transactions relations - same as before)
...
export const showingsRelations = relations(showings, ({ one }) => ({
  ...
  buyer: one(users, { fields: [showings.buyerId], references: [users.id] }),
}));
